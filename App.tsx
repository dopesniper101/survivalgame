...
const App: React.FC = () => {
  ...
  const [gameState, setGameState] = useState<GameState>({
    player: { x: MAP_SIZE / 2, y: MAP_SIZE / 2, health: 100, hunger: 100, thirst: 100, stamina: 100, rotation: 0, isExhausted: false, isCrouching: false },
  ...
  const performAction = useCallback((screenX: number, screenY: number) => {
    ...
    if (clickedNpc && Math.sqrt((clickedNpc.x - currentState.player.x)**2 + (clickedNpc.y - currentState.player.y)**2) < 160) {
      const damage = activeItem?.id.startsWith('hatchet') || activeItem?.id.startsWith('pickaxe') ? 34 : (activeItem?.id.startsWith('rock_tool') ? 15 : 5);
      ...
      setGameState(prev => {
        ...
        if (isFatal) {
          switch(clickedNpc.type) {
            case 'wolf': addLoot({ id: 'meat_wolf', name: 'Wolf Meat', icon: '🍖', count: 2, type: 'consumable' }); addLoot({ id: 'cloth', name: 'Cloth', icon: '🧵', count: 5, type: 'resource' }); break;
            case 'bear': addLoot({ id: 'meat_bear', name: 'Bear Meat', icon: '🥩', count: 6, type: 'consumable' }); addLoot({ id: 'cloth', name: 'Cloth', icon: '🧵', count: 12, type: 'resource' }); break;
            case 'boar': addLoot({ id: 'meat_boar', name: 'Boar Meat', icon: '🥓', count: 4, type: 'consumable' }); addLoot({ id: 'cloth', name: 'Cloth', icon: '🧵', count: 3, type: 'resource' }); break;
            case 'chicken': addLoot({ id: 'meat_chicken', name: 'Raw Chicken', icon: '🍗', count: 1, type: 'consumable' }); break;
          }
        }
    ...
    if (clickedEntity && Math.sqrt((clickedEntity.x - currentState.player.x)**2 + (clickedEntity.y - currentState.player.y)**2) < 160) {
      ...
      const isCorrectTool = (activeItem?.id.startsWith('hatchet') && resType === 'wood') || (activeItem?.id.startsWith('pickaxe') && resType === 'stone');
      ...
      setGameState(prev => {
        const damage = isGround ? 100 : (isCorrectTool ? 34 : (isRock ? 15 : 5));
        const gain = isGround ? 5 : (isCorrectTool ? 12 : (isRock ? 3 : 1));
        let newHealth = prev.player.health;
        if (isPunching) newHealth = Math.max(0, newHealth - HAND_HARVEST_DAMAGE);
        let newInv = [...prev.inventory];
        const resIdx = newInv.findIndex(i => i.id === resType);
        if (resIdx > -1) newInv[resIdx].count += gain;
        else newInv.push({ id: resType, name: resType.toUpperCase(), type: 'resource', count: gain, icon: resType === 'wood' ? '🪵' : '🪨' });
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (gameStatus === 'playing') {
        ...
        if (['1','2','3','4','5','6'].includes(key)) {
          ...
          const hotbarItems = gameStateRef.current.inventory.filter(i => ['tool', 'building', 'consumable'].includes(i.type));
      ...
      if (delta >= TICK_RATE) {
        ...
        setGameState(prev => {
          ...
          const updatedNpcs = prev.npcs.map(npc => {
            ...
            if (npc.type === 'bear' || npc.type === 'wolf') { if (dist < 350) nState = 'chasing'; else nState = 'idle'; }
            else if (npc.type === 'boar' || npc.type === 'chicken') { if (dist < 150) nState = 'fleeing'; else nState = 'idle'; }
            ...
            if (nState === 'chasing') {
              ...
              if (dist < NPC_SAFE_DISTANCE + 15) { health -= (npc.type === 'wolf' ? 0.15 : 0.3); }
            } else if (nState === 'fleeing') { nRot = Math.atan2(ny - y, nx - x); nextX += Math.cos(nRot) * 4; nextY += Math.sin(nRot) * 4; }
            else { nextX += Math.cos(nRot) * 0.5; nextY += Math.sin(nRot) * 0.5; if (Math.random() < 0.02) nRot += (Math.random() - 0.5) * 1; }
  ...
  const handleCraft = (recipe: Recipe) => {
    setGameState(prev => {
      const newInv = [...prev.inventory];
      if (!recipe.ingredients.every(ing => (newInv.find(i => i.id === ing.type)?.count || 0) >= ing.count)) return prev;
      recipe.ingredients.forEach(ing => { const idx = newInv.findIndex(i => i.id === ing.type); newInv[idx].count -= ing.count; });
      const out = { ...recipe.output };
      if (out.type === 'tool') out.id = `${out.id}-${Date.now()}`;
      const existing = newInv.findIndex(i => i.id === out.id);
      if (existing > -1 && out.type !== 'tool') newInv[existing].count += out.count;
  ...
  if (gameStatus === 'dead') return (
    <div className="flex flex-col items-center justify-center h-screen bg-[#0c0a09] text-white">
      ...
      <button onClick={() => window.location.reload()} className="px-12 py-4 bg-orange-700 font-black rounded-sm border-b-8 border-orange-950 active:translate-y-2 active:border-b-0 transition-all uppercase tracking-widest">Respawn</button>
    </div>
  );
